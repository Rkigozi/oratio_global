...
import { getSubmittedPrayers } from "../data/profile-data";
import { PrayerRow } from "./profile";

export function SubmittedPage() {
  const navigate = useNavigate();
  const prayers = useMemo(() => getSubmittedPrayers(), []);
    ...
    <div className="w-full h-full flex flex-col" style={{ background: "#0A1A3A" }}>
      ...
      <div className="relative z-10 px-5 pt-8 pb-24 overflow-y-auto flex-1 h-full">
        <div className="max-w-md mx-auto">
          {/* Header */}
          <div className="mb-6">
            <h2 className="text-[#e2e4f0] font-heading text-lg mb-2">Your Submitted Prayers</h2>
            <p className="text-[#6b7499] text-sm">
              {prayers.length} {prayers.length === 1 ? 'prayer' : 'prayers'} submitted
            </p>
          </div>
          ...
          <div className="space-y-2.5">
            {prayers.length > 0 ? (
              ...
              >
                <Send size={24} className="text-[#4e5573] mx-auto mb-3" />
                <p className="text-[#6b7499] text-sm mb-1">No prayers submitted yet</p>
