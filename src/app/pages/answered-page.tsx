...
import { getAnsweredPrayers } from "../data/profile-data";
import { PrayerRow } from "./profile";

export function AnsweredPage() {
  const navigate = useNavigate();
  const prayers = useMemo(() => getAnsweredPrayers(), []);
    ...
    <div className="w-full h-full flex flex-col" style={{ background: "#0A1A3A" }}>
      ...
      <div className="relative z-10 px-5 pt-8 pb-24 overflow-y-auto flex-1 h-full">
        <div className="max-w-md mx-auto">
          {/* Header */}
          <div className="mb-6">
            <h2 className="text-[#e2e4f0] font-heading text-lg mb-2">Answered Prayers</h2>
            <p className="text-[#6b7499] text-sm">
              {prayers.length} {prayers.length === 1 ? 'prayer' : 'prayers'} answered
            </p>
          </div>
          ...
          <div className="space-y-2.5">
            {prayers.length > 0 ? (
              ...
              >
                <Sparkles size={24} className="text-[#4e5573] mx-auto mb-3" />
                <p className="text-[#6b7499] text-sm mb-1">No answered prayers yet</p>
                <p className="text-[#4e5573] text-xs">
                  Mark your prayers as answered to celebrate God's faithfulness
                </p>
                <button
                  onClick={() => navigate('/profile')}
                  className="mt-4 px-5 py-2.5 rounded-full text-sm text-[#fbbf24] bg-[rgba(251,191,36,0.08)] border border-[rgba(251,191,36,0.12)] cursor-pointer hover:bg-[rgba(251,191,36,0.12)] transition-all"
                >
                  View Your Prayers
                </button>
              </motion.div>
