...
import { saveProfile, markUsernameUsed } from "../data/profile-data";
  ...
  return (
    <div className="flex flex-col min-h-screen w-full text-[#e8eaf6] relative overflow-hidden" style={{ background: "#0A1A3A" }}>
      ...
      <div className="relative z-10 flex flex-col flex-1 justify-center px-6 max-w-sm mx-auto w-full">
        ...
        >
          <p className="text-[#7c8fff] text-xs tracking-[0.25em] uppercase mb-4">Welcome to</p>
          <h1 className="font-heading font-light tracking-[0.2em] text-white mb-3" style={{ fontSize: "2.2rem" }}>
            ORATIO
          </h1>
          <p className="text-[#6b7499] text-sm">Who's praying today?</p>
        </motion.div>
        ...
        >
          <p className="text-[#8890b5] text-sm text-center mb-6 leading-relaxed">
            Oratio connects people around the world through prayer. Choose your username to begin.
          </p>
          ...
          <input type="text" value={username}
            ...
            onBlur={() => { const s = sanitizeUsernameInput(username); setUsername(s); }}
          ...
          />
          {errors.username && (
            <p className="text-[#ff6b6b] text-xs text-center mt-2">{errors.username}</p>
          )}
        </motion.div>
