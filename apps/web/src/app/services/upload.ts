...
export type UploadAvatarResult = { url: string; error: null } | { url: null; error: string };
...
const BROWSER_FRIENDLY_IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp']);
...
export async function uploadAvatar(
  ...
  try {
    ...
    const { error } = await withTimeout(
      supabase.storage.from(AVATAR_BUCKET).upload(path, uploadFile, {
        cacheControl: '3600',
        contentType,
        upsert: true,
      }),
...
type PreparedAvatarResult = { file: File; error: null } | { file: null; error: string };
...
async function convertHeicToJpeg(file: File): Promise<PreparedAvatarResult> {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    return {
      file: null,
      error:
        'That iPhone photo could not be converted here. Try a screenshot, JPEG, PNG, or WebP image instead.',
    };
  }
  ...
  try {
    ...
    return {
      file: null,
      error:
        'That iPhone photo could not be converted. Try a screenshot, JPEG, PNG, or WebP image instead.',
    };
  }
}
